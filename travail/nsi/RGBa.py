...
def test_RGB(nuplet):
    return len(nuplet)==4 and all([ (isinstance(v,int) and v >= 0 and v<=255) for v in nuplet])
...
def toclhex(quadruplet):
    if not test_RGB(quadruplet):
        #print("ERROR : votre quadruplet n'est pas un bon quadruplet RGB") #si vous voulez un affichage d'erreur
        return "#000000"
    return "#"+"".join( [ v+"0" if len(v)==1 else v for v in [tohex(val) for val in quadruplet] ] )

def toclrgb(clhex):
    assert len(clhex)==9
    return (todec(clhex[1:3]),todec(clhex[3:5]),todec(clhex[5:7]),todec(clhex[7:9]))
...
def main():
    ...
    if len(args)<=1:
        ...
        if(args[1]=="tohex"):
            ...
            if(len(tt))==4:
                ...
                print("EH ! Il me faut un tuple de 4 chiffres entre 0 et 255 séparés par une virgule ok ?")
        elif(args[1]=="torgba"):
            tt=args[2].lower()
            clhex="#"+tt
            clrgb=toclrgb(clhex)
    ...
    if pasbon:
        print("""
Les commandes sont :
    -'torgba couleur hexadecimale sans le dieses, juste les 8 characteres'
  et
    'tohex quadruplet rgba sans les parentheses et separes par des virgules'
""")
